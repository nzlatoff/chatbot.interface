...
socket.on("bot config from server", (data) => {
	...
	const textareaFields = ["character", "subtext", "first_words"];

	const optionsFields = ["mode"];
	...
	const subData = Object.entries((({ model, run }) => ({ model, run }))(data))
		.join(" | ")
		.replace(/,/g, ": ");
	document.querySelector(`#title-wrapper-${data.id}`).innerHTML +=
		`<small>${subData}</small>`;
...
socket.on("received batch", (data) => {
	...
	let batch_controls = document
		.getElementById(id)
		.querySelector(".batch-controls");
	if (!batch_controls) {
	...
	batch_messages_form.addEventListener(
		"dblclick",
		(e) => {
			submitMessage(id);
		},
...
socket.on("server confirms bot choice", (data) => {
	...
	flashBtn(
		document.querySelector(`#batch-input-${data.id}-${data.choice}`).parentNode,
		data.id,
	);
});
...
document.querySelector("#reset-button").addEventListener("click", () => {
	...
	Array.from(document.querySelectorAll(".character-wrapper textarea")).forEach(
		(e) => {
			e.placeholder = "";
		},
	);
	Array.from(document.getElementsByClassName("batch-controls")).forEach((e) =>
		e.remove(),
	);
...
document.body.onkeyup = (e) => {
	...
	if (
		(e.keyCode === 32 || e.key === " ") &&
		!["textarea", "input"].includes(document.activeElement.localName)
	) {
